...
i=0
for directory in $(find ./nixos -maxdepth 1 -mindepth 1 -type d | sort | sed "s/\.\/nixos\///"); do
    echo "[$i] $directory"
    i=$((i+1))
...
if [ -z "$CONFIGURATION" ]; then
    ...
    sudo cp $SCRIPT_DIR/nixos/$CONFIG/configuration.nix /mnt/etc/nixos/configuration.nix
fi
